...
GitHub Actions runs every Monday with cron `17 2 * * 1` (UTC), installs `.[dev]`, runs `python scripts/weekly.py`, and commits generated changes under `data/export` and `data/reports`.

The workflow can also be started manually with optional `week` and `force` inputs. `week` accepts an ISO week key like `2026-W26`; `force` overwrites existing generated files. When `week` is omitted (scheduled default), the pipeline automatically processes the previous completed ISO week in `Europe/Madrid`.
